...
<template>
  ...
  <ul id="side-menu" class="metismenu">
    ...
    >
      ...
      <vue-fontawesome v-if="item.icon" :icon="item.icon" size="1" ></vue-fontawesome>
        <!-- <feather v-if="item.icon" :type="item.icon"></feather> -->
        <!-- <i v-if="item.icon" :class="item.ficon"></i> -->
        <span>{{ item.name }}</span>
        <span
          v-if="item.badge"
          :class="
    ...
    <span @click="logout()" class="logout" v-b-tooltip.hover title="Logout"><vue-fontawesome icon="power-off" size="1.7" class="logout_icon"></vue-fontawesome>
    </span>
